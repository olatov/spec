...
void main() {
    ...
    // A BW tube is a single continuous phosphor coating with one electron
    // gun - there's no colour-separating shadow mask (that only exists to
    // keep a colour tube's R/G/B beams on their own phosphor stripe). All
    // that's left to see is a faint brightness ripple from the tube's
    // grille structure, so modulate luminance only - no colour fringing.
    float texelX = uv.x * float(textureSize(texture0, 0).x);
    float t = texelX * 6.28318530718;
    float mask = 0.85 + 0.15 * cos(t);

    float aa = fwidth(t);
    float fade = clamp(1.0 - aa / 3.14159265, 0.0, 1.0);

    color.rgb *= mix(1.0, mask, 0.45 * fade);
